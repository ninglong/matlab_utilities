...
% fig.columns=addlParms.columns;
% For each figure
for i=1:fig.count
    % Determine what row and column the current figure is on
    fig_row_num(i) = floor((i-1)/addlParms.columns);
    fig_col_num(i) = mod(i-1,addlParms.columns);
    figpos = get(addlParms.figure(i), 'Position');
    figwidth(i) = figpos(3);
    figheight(i) = figpos(4);
end

for i=1:fig.count
    row = fig_row_num(i);
    column = fig_col_num(i);
    width_scaler = figwidth(i)/sum(figwidth(fig_row_num==fig_row_num(i)));
    height_scaler = figheight(i)/sum(figheight(fig_col_num==fig_col_num(i)));
...
%             set(pic1,'Width',(slide_W-(left+right))/fig.columns);
              set(pic1,'Width',(slide_W-(left+right))*width_scaler);
...
%             if ((slide_H)*height_scaler)/((slide_W)*width_scaler)>(pic_H+(top+bottom))/(pic_W+(left+right))
            if ((slide_H)*height_scaler)/((slide_W)*width_scaler)>(pic_H)/(pic_W)
%                 set(pic1,'Width',(slide_W-(left+right))/fig.columns);
                  set(pic1,'Width',(slide_W-(left+right))*width_scaler);
            else
%                 set(pic1,'Height',(slide_H-(top+bottom))/fig.rows);
                  set(pic1,'Height',(slide_H-(top+bottom))*height_scaler);
            end
        end
    end
